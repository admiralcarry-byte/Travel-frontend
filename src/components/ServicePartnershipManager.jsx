...
const ServicePartnershipManager = ({ service, onPartnershipUpdate }) => {
  ...
  const fetchExchangeRate = async (from, to) => {
    try {
      ...
      const response = await api.get(`/api/payments/exchange-rate?from=${from}&to=${to}`);
      if (response.data.success) {
  ...
  const handleCostChange = async (value) => {
    ...
    if (newPartnership.currency !== 'USD' && amount > 0) {
      await convertCurrency(amount, newPartnership.currency, 'USD');
    } else {
      setConvertedAmount(amount);
    }
  };
  ...
  const handleCurrencyChange = async (currency) => {
    ...
    if (newPartnership.costProvider > 0) {
      await convertCurrency(newPartnership.costProvider, currency, 'USD');
    } else {
